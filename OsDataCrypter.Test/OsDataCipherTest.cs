...
namespace OsDataCipher.Test
{
    public class OsDataCipherTest
    {
        [Theory]
        ...
        public void Encrypt_And_Decrypt_Should_Obtain_Same_String(string str, string password)
        {
            ...
            string enc = osDataCipher.Encrypt(str);
            string dec = osDataCipher.Decrypt(enc);

            Assert.NotNull(enc);
            Assert.NotEqual(enc, string.Empty);
        ...
        public void Encrypt_And_Decrypt_With_Different_Instance_Should_Obtain_Same_String()
        {
            ...
            string enc = osDataCipher.Encrypt(str);
            string dec = osDataCipher2.Decrypt(enc);

            Assert.Equal(str, dec);
        }

        [Fact]
        public void Different_Password_Should_Return_An_Exception()
        {
            string str = "data to encrypt";
            IOsDataCipher osDataCipher = new OsDataCipher("password");
            IOsDataCipher osDataCipher2 = new OsDataCipher("different");

            string enc = osDataCipher.Encrypt(str);

            Assert.ThrowsAny<Exception>(()=> osDataCipher2.Decrypt(enc));
        }

        [Fact]
        public void Encrypt_And_Decrypt_Big_String_Should_Obtain_Same_Big_String()
        {
            string str = @"
{
  ""web-app"": {
